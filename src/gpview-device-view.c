...
enum
{
    GPVIEW_DEVICE_VIEW_DEVICE_ID = 1,
    GPVIEW_DEVICE_VIEW_CONTROLLER,
    GPVIEW_DEVICE_VIEW_DATABASE,
    GPVIEW_DEVICE_VIEW_RESULT,
    GPVIEW_DEVICE_VIEW_WEBSITES
};

typedef struct _GPViewDeviceViewPrivate GPViewDeviceViewPrivate;
...
static void
gpview_device_view_activate(GPViewViewInterface *widget);

static void
gpview_device_view_changed_cb(GtkTreeSelection *selection, GPViewDeviceView *view);

static void
gpview_device_view_class_init(gpointer g_class, gpointer g_class_data);

static void
gpview_device_view_deactivate(GPViewViewInterface *widget);

static void
gpview_device_view_dispose(GObject *object);

static void
gpview_device_view_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec);

static void
gpview_device_view_init(GTypeInstance *instance, gpointer g_class);

static void
gpview_device_view_init_view_interface(gpointer iface, gpointer user_data);

static void
gpview_device_view_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec);

static void
gpview_device_view_set_result(GPViewDeviceView *view, GPartsDatabaseResult *result);

static void
gpview_device_view_update_cb(GObject *unused, GParamSpec *pspec, GPViewDeviceView *view);
...
gpview_device_view_activate(GPViewViewInterface *widget)
{
    GPViewDeviceView *view = GPVIEW_DEVICE_VIEW(widget);
    ...
    else
    {
        ...
        if (privat == NULL)
        {
            g_critical("Unable to obtain private data for a GPViewDeviceView");
        }
...
gpview_device_view_class_init(gpointer g_class, gpointer g_class_data)
{
    ...
    g_type_class_add_private(g_class, sizeof(GPViewDeviceViewPrivate));

    klasse->dispose = gpview_device_view_dispose;

    klasse->get_property = gpview_device_view_get_property;
    klasse->set_property = gpview_device_view_set_property;

    g_object_class_install_property(
        klasse,
        GPVIEW_DEVICE_VIEW_DEVICE_ID,
        g_param_spec_string(
            "company-id",
            "Company ID",
            "Company ID",
...
gpview_device_view_dispose(GObject *object)
{
    GPViewDeviceViewPrivate *privat = GPVIEW_DEVICE_VIEW_GET_PRIVATE(object);
...
gpview_device_view_get_type(void)
{
    ...
    if (type == G_TYPE_INVALID)
    {
        static const GTypeInfo tinfo = {
            sizeof(GPViewDeviceViewClass),    /* class_size */
            NULL,                              /* base_init */
            NULL,                              /* base_finalize */
            gpview_device_view_class_init,    /* class_init */
            NULL,                              /* class_finalize */
            NULL,                              /* class_data */
            sizeof(GPViewDeviceView),         /* instance_size */
            0,                                 /* n_preallocs */
            gpview_device_view_init,          /* instance_init */
            NULL                               /* value_table */
            };

        static const GInterfaceInfo iinfo = {
            gpview_device_view_init_view_interface,    /* interface_init */
...
GStrv
gpview_device_view_get_websites(GPViewDeviceView *view)
{
    GPViewDeviceViewPrivate *privat = GPVIEW_DEVICE_VIEW_GET_PRIVATE(view);
    GStrv websites = NULL;
...
gpview_device_view_init(GTypeInstance *instance, gpointer g_class)
{
    GPViewDeviceViewPrivate *privat = GPVIEW_DEVICE_VIEW_GET_PRIVATE(instance);
    ...
    if (privat != NULL)
    {
        ...
        g_signal_connect(
            privat->selection,
            "changed",
            G_CALLBACK(gpview_device_view_changed_cb),
            instance
            );
        ...
        g_signal_connect(
            instance,
            "notify::database",
            G_CALLBACK(gpview_device_view_update_cb),
            instance
            );
    }
}
...
gpview_device_view_init_view_interface(gpointer iface, gpointer user_data)
{
    ...
    iface2->activate   = gpview_device_view_activate;
    iface2->deactivate = gpview_device_view_deactivate;
}

GPViewDeviceView*
gpview_device_view_new()
{
...
{
    return GPVIEW_DEVICE_VIEW(g_object_new(
...
gpview_device_view_set_controller(GPViewDeviceView *view, GPViewDeviceCtrl *ctrl)
{
    ...
    if (privat != NULL)
    {
        if (privat->controller != NULL)
        {
            ...
            //    privat->database,
            //    G_CALLBACK(gpview_device_view_notify_connected_cb),
            //    view
            //    );
...
gpview_device_view_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    GPViewDeviceView *view = GPVIEW_DEVICE_VIEW(object);
...
gpview_device_view_update_cb(GObject *unused, GParamSpec *pspec, GPViewDeviceView *view)
{
    ...
    if (privat != NULL)
    {
        if (privat->database != NULL)
        {
            ...
            result = gparts_database_query(privat->database, "SELECT * FROM DeviceV", NULL);

            gpview_device_view_set_result(view, result);
